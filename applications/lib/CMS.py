...
class CMS:
    ...
    def connect(self):
        ...
        try:
            ...
            print('Error occurred when establishing connection with CMS!')
            return False
    ...
    def auth(self):
        if self.conn != None:
            try:
                ...
                conn.request('POST', self.path+'/j_security_check', body, headers=headers)
                r1 = conn.getresponse()
                resp = r1.read().decode("utf-8")
                if r1.status == 302:
                    conn.request('GET', self.path+'/auth.json', None, headers)
